...
class JogoControllerSpec extends Specification {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.jogoService = Mock(JogoService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.jogoService = Mock(JogoService) {
            1 * save(_ as Jogo) >> { Jogo jogo ->
                throw new ValidationException("Invalid instance", jogo.errors)
            }
        }
